...
> Bản checklist này đã được trim về đúng scope MVP bảo thủ: COD-first, không payment gateway, không social login, không kéo Phase 2 vào execution checklist.
...
- [ ] Cài đặt dependencies:
  ...
  factory-boy
  ```
- [ ] Khóa dependency source qua `pyproject.toml`
- [ ] Khởi tạo Django project: `django-admin startproject config .`
- [ ] Tạo các apps cốt lõi: `accounts`, `products`, `orders`
...
- [ ] Với COD MVP, giữ dữ liệu thanh toán tối thiểu trong `Order` hoặc snapshot liên quan; không mở app `payments` riêng nếu chưa thực sự cần
...
- [ ] Đổi URL admin từ `/admin/` sang `/secret-panel/` (bảo mật)
...
- [ ] Tạo `Dockerfile` cho Django (production-ready):
  ...
  - [ ] Chạy với server production phù hợp với cách deploy hiện tại
...
- [ ] API `POST /api/orders/`:
  - Tạo order ở `PENDING`, chưa trừ tồn kho
  - Tạo `Order` + `OrderItem`
  - Trả về order ID
...
- [ ] Admin API `POST /api/admin/orders/{id}/confirm/`:
  - Chỉ hợp lệ khi order đang `PENDING`
  - Dùng `select_for_update()` để lock variant
  - Kiểm tra tồn kho toàn bộ items
  - Trừ tồn kho và chuyển `PENDING -> CONFIRMED`
  - Nếu thiếu kho: rollback toàn bộ, trả `OUT_OF_STOCK`, giữ order ở `PENDING`
- [ ] Admin API `POST /api/admin/orders/{id}/ship/` (chỉ khi `CONFIRMED`)
- [ ] Admin API `POST /api/admin/orders/{id}/deliver/` (chỉ khi `SHIPPED`)
- [ ] Admin API `POST /api/admin/orders/{id}/cancel/`:
  - Hợp lệ khi `PENDING` hoặc `CONFIRMED`
  - Chỉ restock khi hủy từ `CONFIRMED`
- [ ] Xử lý race condition: 2 người cùng mua sản phẩm cuối cùng
  - Dùng `select_for_update()` + database transaction ở bước admin confirm
...
- [ ] Luồng COD hoàn chỉnh (backend):
  ...
  - [ ] `POST /api/admin/orders/{id}/confirm/` mới là nơi trừ tồn kho:
  ...
  - [ ] Confirm gọi sai state phải trả `ORDER_INVALID_STATE`
...
- [ ] Custom DRF exception handler — mọi lỗi đều trả JSON chuẩn:
  ...
  ```python
  # {"code": "OUT_OF_STOCK", "message": "...", "errors": {"items": ["..."]}}
  ```
...
- [ ] HTML email template "Đã nhận yêu cầu đặt hàng":
  - Logo, tên khách hàng
  - Danh sách sản phẩm đã mua
  - Tổng tiền, địa chỉ giao hàng
  - Mã đơn hàng + link xem đơn
- [ ] Service gửi email “đã nhận yêu cầu đặt hàng / chờ xác nhận” ngay sau khi tạo đơn COD thành công
...
- [ ] Custom action: "Đánh dấu đã giao vận" (`SHIPPED`)
- [ ] Custom action: "Đánh dấu đã giao thành công" (`DELIVERED`)
...
- [ ] Test end-to-end luồng hoàn chỉnh (thủ công):
  ...
  4. Quay về trang success → nhận email “đã nhận yêu cầu đặt hàng / chờ xác nhận”
  5. Vào `/account/orders` xem đơn
  6. Admin xác nhận đơn → cập nhật trạng thái
- [ ] Test hủy đơn hàng → tồn kho hoàn lại đúng
- [ ] Test confirm thiếu kho → order vẫn `PENDING`, không trừ kho item nào
...
- [ ] Email “đã nhận yêu cầu đặt hàng / chờ xác nhận” gửi được thật (không chỉ console log)
...
> - ✅ Email “đã nhận yêu cầu đặt hàng / chờ xác nhận” được gửi đến địa chỉ email của khách
> - ✅ Đơn hàng xuất hiện trong Django Admin với status `PENDING`
> - ✅ Chỉ khi admin confirm order thì tồn kho mới giảm đúng số lượng đã đặt
> - ❌ Nếu admin confirm khi sản phẩm không đủ hàng → trả `OUT_OF_STOCK`, không trừ kho phần nào và order vẫn ở `PENDING`
...
- [ ] Tests cho `orders`:
  ...
  - [ ] `test_create_order_keeps_order_pending_without_deducting_stock`
  - [ ] `test_confirm_order_reduces_stock`
  - [ ] `test_confirm_order_fails_when_out_of_stock_and_keeps_pending`
...
- [ ] Test toàn bộ luồng trên production (điện thoại thật, không dùng localhost):
  - [ ] Luồng 1: Khách mới → đăng ký → mua hàng COD → nhận email “đã nhận yêu cầu đặt hàng / chờ xác nhận”
  - [ ] Luồng 2: Đăng nhập → tìm kiếm → lọc → thêm giỏ → checkout COD → nhận email “đã nhận yêu cầu đặt hàng / chờ xác nhận”
