...
| **Auth**           | SimpleJWT                              | Gọn cho MVP, social login defer sang Phase 2   |
...
- Checkout page với React Hook Form + Zod:
  ...
  - Bước 3: Xác nhận đơn
- API tạo đơn hàng (`POST /api/orders/`) chỉ tạo `PENDING`, chưa commit inventory:
...
- Admin `confirm order` mới là nơi lock variant, kiểm tra tồn kho, trừ kho, và chuyển `PENDING -> CONFIRMED`
- Error handling: confirm fail vì thiếu kho thì rollback toàn bộ và giữ order ở `PENDING`

**Ngày 18–19: Order Confirmation + Email**

- Trang `/orders/:id` — chi tiết đơn hàng
- Gửi email “đã nhận yêu cầu đặt hàng / chờ xác nhận” qua Django (HTML template)
...
- Luồng COD: tạo order → status `PENDING` → redirect trang success → gửi email đã nhận yêu cầu
- Trang `/checkout/success` và `/checkout/failed`
- Custom action trong Admin: xác nhận / đánh dấu đã giao vận / đánh dấu đã giao thành công / hủy đơn hàng hàng loạt
...
- [ ] Email “đã nhận yêu cầu đặt hàng / chờ xác nhận” gửi được thật (không chỉ log)
...
- Test confirm tồn kho + race condition (`select_for_update`)
...
✅ Khách nhận email đã nhận yêu cầu đặt hàng
...
_Xem tổng quan chức năng tại: [overview.md](./overview.md)_
