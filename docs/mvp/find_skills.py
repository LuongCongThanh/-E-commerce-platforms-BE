...
def match_skill(keyword: str, skill_name: str) -> str | None:
    ...
    if (
        (kw in sn or sn in kw)
        and (kw, sn) not in _AMBIGUOUS
        and (sn, kw) not in _AMBIGUOUS
    ):
        return "contains"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["vi", "en"],
        default="vi",
        help="Output language (default: vi)",
    )
        ...
        for sd in skill_dirs:
            ...
            if match_type:
                entry = skill_map.setdefault(
                    sd.name, {"keywords": set(), "match_type": match_type, "path": sd}
                )
    ...
    for domain_primary, keywords_str, link, domains, summary in rows:
        if domain_primary != current_domain:
            ...
            out.append("|---|---|---|---|\n")
        cells = " | ".join(
            escape_cell(c) for c in (keywords_str, link, domains, summary)
        )
        out.append(f"| {cells} |\n")
