...
> [!WARNING]
> Legacy forward-looking note. The `/api/v1/...` strategy below is not the active canonical contract unless it is explicitly re-adopted.
> Use `docs/be/01-mvp-overview.vi.md` for the current API baseline and `docs/be/06-documentation-consistency-audit.vi.md` for versioning conflict context.
