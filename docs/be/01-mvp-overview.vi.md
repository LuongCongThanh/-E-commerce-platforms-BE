...
Source of truth: `CONTEXT.md`, `docs/adr/0001-order-inventory-lifecycle.md`, `docs/mvp/mvp-plan.md`, Django project structure, API contracts
...
- Cart/Inventory: tạo order ở `PENDING`, commit tồn kho khi admin xác nhận đơn (`CONFIRMED`) bằng atomic transaction + `select_for_update()`.
- Order API: tạo đơn COD, xem lịch sử đơn, chi tiết đơn — phía khách hàng.
- Admin API / Django Admin: CRUD sản phẩm, quản lý đơn hàng, action `confirm`, `ship`, `deliver`, `cancel`.
- Email: gửi email xác nhận đã nhận yêu cầu đặt hàng khi order được tạo ở `PENDING`.
...
- Tất cả lỗi API response phải theo canonical shape `{ code, message, errors }`.
- COD-first: không tích hợp payment gateway trong MVP.
- `POST /api/orders/` chỉ tạo order ở `PENDING`, chưa commit tồn kho.
- Commit inventory chỉ xảy ra tại action admin `confirm order` (`PENDING -> CONFIRMED`).
- Atomic transaction cho confirm/cancel order có tác động tồn kho — không cho phép oversell.
...
3. Order flow:
   - `POST /api/orders/` → tạo đơn COD ở trạng thái `PENDING`.
   - `GET /api/orders/` → lịch sử đơn của user đang đăng nhập.
   - `GET /api/orders/{id}/` → chi tiết đơn.
   - `POST /api/orders/{id}/cancel/` → customer chỉ được hủy order của chính mình khi order còn `PENDING`.
4. Admin flow (Django Admin):
   - CRUD sản phẩm, variant, hình ảnh.
   - Xem danh sách đơn, lọc theo trạng thái, action `confirm`, `ship`, `deliver`, `cancel`.
...
POST /api/orders/           → Order { id, status: PENDING, total, items[], shipping_address }
...
POST /api/orders/{id}/cancel/ → Order { id, status: CANCELLED, ... } hoặc lỗi `ORDER_INVALID_STATE`
...
- Data integrity:
  - `select_for_update()` bắt buộc trong admin `confirm order`.
  - Nếu confirm thiếu tồn kho: rollback toàn bộ, trả `OUT_OF_STOCK`, giữ order ở `PENDING`.
  - Customer cancel chỉ hợp lệ khi order đang `PENDING`.
  - Admin cancel hợp lệ khi order đang `PENDING` hoặc `CONFIRMED`; chỉ restock khi hủy từ `CONFIRMED`.
...
- Ops KPIs:
  ...
  - Không có case double stock deduction khi confirm nhiều lần.
  - Đơn hàng COD được tạo ở `PENDING` thành công > 99%.
- Quality KPIs:
...
- FE có thể chạy end-to-end COD flow với đơn được tạo ở `PENDING` và hiển thị đúng trạng thái “đã nhận yêu cầu / chờ xác nhận”.
...
- Race condition khi admin confirm nhiều đơn cùng variant hết hàng → mitigation bằng `select_for_update`.
