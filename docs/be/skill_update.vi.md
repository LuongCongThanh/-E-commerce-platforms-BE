...
| Dependencies     | `security-scanning-security-dependencies` | Scan CVE trong `pyproject.toml` hoặc manifest tương đương, supply chain risks, license check | Audit manifest/dependencies rồi triage upgrade an toàn             | Update dependency, hardening release, monthly security maintenance  | High            |
