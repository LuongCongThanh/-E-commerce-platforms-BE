...
- Nếu docs cũ mâu thuẫn phiên bản, ưu tiên `pyproject.toml`.
...
- `pyproject.toml` là dependency source hiện tại của repo.
...
- Version matrix phản ánh đúng `pyproject.toml` hiện tại hoặc ghi rõ những dependency mới chỉ là planned.
...
- [ ] Thêm check định kỳ đối chiếu version docs với `pyproject.toml`.
