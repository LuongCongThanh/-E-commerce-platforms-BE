...
Source of truth: `CONTEXT.md`, `docs/adr/0001-order-inventory-lifecycle.md`, `01-mvp-overview.vi.md`, `docs/mvp/mvp-plan.md`
...
1. MVP (Phase 0):
   - Outcomes: auth API + catalog API + order API COD (`PENDING` first) + Django Admin confirm/ship/deliver/cancel + email xác nhận đã nhận yêu cầu đặt hàng.
   - Timebox: 4 tuần.
2. Post-MVP Phase 1:
...
- Ngày 1–2: Project setup
  ...
  - Cài DRF, drf-spectacular, SimpleJWT.
...
- Ngày 10–11: Order API
  - `POST /api/orders/` — create order ở `PENDING`, chưa deduct stock.
  - Trả `Order` shape chuẩn theo contract.
  - `GET /api/orders/` + `GET /api/orders/{id}/` — chỉ trả data của user đang đăng nhập.
  - `POST /api/orders/{id}/cancel/` — customer chỉ cancel order của chính mình khi còn `PENDING`.
- Ngày 12–13: Email + Cloudinary
  - Django email backend (SMTP Gmail/Mailgun).
  - HTML email template: đã nhận yêu cầu đặt hàng / đơn đang chờ xác nhận.
  - Gửi email sync ngay sau khi create order (sync OK ở MVP — defer Celery sang Phase 2).
  - Cloudinary: cấu hình `DEFAULT_FILE_STORAGE` hoặc xử lý URL ảnh.
- Ngày 14: Hardening + Test coverage
  - Viết pytest tests cho order flow: create `PENDING`, confirm stock commit, cancel/restock, race condition ở `confirm`.
  - Verify email gửi được thật trên staging.
  - Seed data đủ để FE test: 5 categories, 20 products, 3 variants mỗi product.

**Gate cuối Tuần 2:**

- [ ] `POST /api/orders/` tạo đơn `PENDING` thành công trên staging.
...
Goal: Django Admin đủ mạnh để vận hành lifecycle `PENDING -> CONFIRMED -> SHIPPED -> DELIVERED`, API ổn định cho FE tích hợp hoàn chỉnh.

- Ngày 15–16: Django Admin nâng cao
  - Custom action: confirm order, mark shipped, mark delivered, cancel order (bulk).
...
- Ngày 17–18: Cập nhật trạng thái đơn API
  - `POST /api/admin/orders/{id}/confirm/` — chỉ admin role, commit inventory tại đây.
  - `POST /api/admin/orders/{id}/ship/` — chỉ hợp lệ khi order đang `CONFIRMED`.
  - `POST /api/admin/orders/{id}/deliver/` — chỉ hợp lệ khi order đang `SHIPPED`.
  - `POST /api/admin/orders/{id}/cancel/` — admin cancel được ở `PENDING` hoặc `CONFIRMED`; chỉ restock khi cancel từ `CONFIRMED`.
  - Không cho phép nhảy trạng thái bất hợp lệ.
...
- [ ] Các action admin order từ chối non-admin và trả `ORDER_INVALID_STATE` khi gọi sai trạng thái.
...
- Ngày 22–24: Test coverage
  - pytest-django: order creation `PENDING`, confirm stock deduction, cancel/restock, race condition ở confirm.
...
| Race condition oversell   | Dữ liệu sai          | Medium     | `select_for_update()` bắt buộc trong admin confirm order | BE Lead |
